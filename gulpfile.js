...
gulp.task('modernizr', function () {
    return gulp.src('app/vendor/modernizr/modernizr.js')
        .pipe(gulp.dest(p.distJs));
});
...
gulp.task('images', function(cb) {
    gulp.src(['images/*.png', 'images/*.jpg', 'images/*.gif', 'images/*.jpeg', 'images/*.ico']).pipe(imageop({
        optimizationLevel: 5,
        progressive: true,
        interlaced: true
    })).pipe(gulp.dest(p.distImg)).on('end', cb).on('error', cb);
});
...
gulp.task('watch', ['clean'], function () {
    gulp.start(['libs', 'bower-libs', 'browserSync', 'watchTask', 'watchify', 'styles', 'scripts', 'images']);
});
